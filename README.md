...
5. [電路圖](#5-電路圖)
   - [5.1 系統方塊圖](#51-系統方塊圖)
   - [5.2 Pi Pico 接腳全覽圖](#52-pi-pico-接腳全覽圖)
   - [5.3 各模組詳細接線](#53-各模組詳細接線)
6. [開發環境建置](#6-開發環境建置)
7. [逐步實作教程](#7-逐步實作教程)
8. [飛行資料分析](#8-飛行資料分析)
9. [常見問題排查](#9-常見問題排查)
10. [水火箭移植指南](#10-水火箭移植指南)
...
│  ┌──────────┐            ┌──────────────────────────────────────┐   │
│  │  BNO055  │◀─I2C 0x28─▶                                      │   │
│  │  9軸 IMU │            │         Raspberry Pi Pico            │   │
│  └──────────┘            │              RP2040                  │   │
│                           │                                      │   │
│  ┌──────────┐            │  ┌─────────┐      ┌───────────────┐ │   │
│  │  BMP390  │◀─I2C 0x77─▶  │  pio0   │      │    pio1       │ │   │
│  │  氣壓計  │            │  │ SM0→GP7 │      │ SM0→GP2 Srv X1│ │   │
│  └──────────┘            │  │ 狀態LED │      │ SM1→GP3 Srv X2│ │   │
│                           │  │ SM1→GP6 │      │ SM2→GP4 Srv Y1│ │   │
│  ┌──────────┐            │  │ 錄製LED │      │ SM3→GP5 Srv Y2│ │   │
│  │ PA1616D  │◀─UART1────▶│  └─────────┘      └───────────────┘ │   │
│  │  GPS     │  (停用)    │                                      │   │
│  └──────────┘            │  SPI1: GP10(SCK) GP11(MOSI)         │   │
│                           │        GP12(MISO) GP13(CS)          │   │
│  ┌──────────┐            │                                      │   │
│  │ MicroSD  │◀─SPI1─────▶│  GP1 ◀──────── 錄製開關             │   │
│  │  Card    │            │                                      │   │
│  └──────────┘            │  VSYS(39) ◀── LiPo+ ──▶ 伺服機 VCC │   │
│                           │  3V3(36)  ──▶ BNO055/BMP390/SD VCC │   │
│  ┌──────────┐            └──────────────────────────────────────┘   │
│  │   SG90   │◀─PIO1 PWM─────────────────────────────────────────   │
│  │  伺服×4  │                                                       │
│  └──────────┘                                                       │
...
Pi Pico 40 腳位與所有模組的完整對應：

```
                    ┌─────────────────────┐
             GP0  1 ┤                     ├ 40  VBUS  ── (USB 5V，飛行時空接)
        錄製開關  2 ┤ GP1          VSYS   ├ 39  ◀── LiPo+ → 伺服機 VCC
             GND  3 ┤              GND    ├ 38  GND
       Servo X1   4 ┤ GP2        3V3_EN   ├ 37
       Servo X2   5 ┤ GP3        3V3OUT   ├ 36  ──▶ BNO055/BMP390/SD 模組 VCC
       Servo Y1   6 ┤ GP4        ADC_REF  ├ 35
       Servo Y2   7 ┤ GP5          GP28   ├ 34
             GND  8 ┤               GND   ├ 33
       錄製 LED   9 ┤ GP6          GP27   ├ 32
       狀態 LED  10 ┤ GP7          GP26   ├ 31
   → GPS 模組 RX 11 ┤ GP8           RUN   ├ 30
   ← GPS 模組 TX 12 ┤ GP9          GP22   ├ 29
             GND 13 ┤               GND   ├ 28
   → MicroSD CLK 14 ┤ GP10         GP21   ├ 27
  → MicroSD MOSI 15 ┤ GP11         GP20   ├ 26
  ← MicroSD MISO 16 ┤ GP12         GP19   ├ 25
    → MicroSD CS 17 ┤ GP13         GP18   ├ 24
             GND 18 ┤               GND   ├ 23
→ BNO055/BMP SDA 19 ┤ GP14         GP17   ├ 22
→ BNO055/BMP SCL 20 ┤ GP15         GP16   ├ 21
                    └─────────────────────┘
                       Raspberry Pi Pico

  箭頭方向：→ 表示 Pico 輸出，← 表示 Pico 接收
```
...
```
Pi Pico (腳位)          BNO055 模組
──────────────          ───────────
3V3OUT (Pin 36) ──────▶ VCC
GND    (Pin 38) ──────▶ GND
GP14   (Pin 19) ──────▶ SDA
GP15   (Pin 20) ──────▶ SCL
                         ADR ──── GND   ← 決定 I2C 位址 = 0x28
                         INT ──── (不接，未使用)
                         RST ──── (不接，軟體重置)
```

#### B. BMP390 氣壓計模組（V2 啟用，V1 停用）

```
Pi Pico (腳位)          BMP390 模組
──────────────          ───────────
3V3OUT (Pin 36) ──────▶ VCC
GND    (Pin 38) ──────▶ GND
GP14   (Pin 19) ──────▶ SDA   ← 與 BNO055 共用同一 I2C 匯流排
GP15   (Pin 20) ──────▶ SCL
                         SDO ── GND   ← I2C 位址 = 0x77（SDO=GND）
                                       （若 SDO=3V3 則位址為 0x76）
```

> **提示**：BNO055（0x28）與 BMP390（0x77）可共用同一組 SDA/SCL 線路，I2C 匯流排允許多裝置並接。

#### C. MicroSD 卡模組（SPI）

```
Pi Pico (腳位)          MicroSD 模組
──────────────          ────────────
3V3OUT (Pin 36) ──────▶ VCC   ← 請確認模組接受 3.3V（部分模組需 5V）
GND    (Pin 38) ──────▶ GND
GP10   (Pin 14) ──────▶ CLK  (SCLK)
GP11   (Pin 15) ──────▶ MOSI (DI)
GP12   (Pin 16) ◀──────  MISO (DO)
GP13   (Pin 17) ──────▶ CS   (SS)
```
...
```
Pi Pico (腳位)                    SG90/MG90S 伺服機
──────────────                    ─────────────────
VSYS   (Pin 39) ──┬───────────▶ 紅線 VCC  [Servo X1]
                   ├───────────▶ 紅線 VCC  [Servo X2]
                   ├───────────▶ 紅線 VCC  [Servo Y1]
                   └───────────▶ 紅線 VCC  [Servo Y2]

GND    (Pin 38) ──┬───────────▶ 棕線 GND  [Servo X1]
                   ├───────────▶ 棕線 GND  [Servo X2]
                   ├───────────▶ 棕線 GND  [Servo Y1]
                   └───────────▶ 棕線 GND  [Servo Y2]

GP2    (Pin  4) ──────────────▶ 橘線 訊號 [Servo X1]  ← PIO1 SM0
GP3    (Pin  5) ──────────────▶ 橘線 訊號 [Servo X2]  ← PIO1 SM1
GP4    (Pin  6) ──────────────▶ 橘線 訊號 [Servo Y1]  ← PIO1 SM2
GP5    (Pin  7) ──────────────▶ 橘線 訊號 [Servo Y2]  ← PIO1 SM3

  ⚡ 電源說明：
  VSYS = LiPo 直接供電（3.7~4.2V），SG90 可在此電壓正常工作
  VBUS（Pin 40）只在 USB 插入時有 5V，飛行時不可靠，禁止使用
```
...
Pi Pico (腳位)                       LED
──────────────                       ───
GP7    (Pin 10) ──[330Ω]──▶ 🟡 黃色 LED 正極 (+) ──▶ GND  ← 狀態 LED (PIO0 SM0)
GP6    (Pin  9) ──[330Ω]──▶ 🔴 紅色 LED 正極 (+) ──▶ GND  ← 錄製 LED (PIO0 SM1)

  LED 狀態說明：
  ┌──────────────────────────────────────────────────────────┐
  │  狀態 LED 🟡        錄製 LED 🔴      系統狀態            │
  │  快速閃爍（3Hz）    快速閃爍（3Hz）  初始化中            │
  │  慢速閃爍（~0.3Hz） 熄滅             待機，等待錄製      │
  │  慢速閃爍（~0.3Hz） 慢速閃爍（~0.3Hz）正在錄製飛行資料  │
  └──────────────────────────────────────────────────────────┘
```
...
Pi Pico (腳位)                       開關
──────────────                       ────
GP1    (Pin  2) ─────────────── [開關] ──── GND (Pin 38)
  (內部 pull-up 至 3.3V，由軟體設定)

  操作邏輯：
  開關斷開 → GP1 = HIGH（3.3V，pull-up）→ recording = false
  開關閉合 → GP1 = LOW（接 GND）→ recording = true  ← 開始錄製
  再次閉合→斷開（關掉再開）→ 建立新的 IMU_x.csv 檔案
```

#### G. PA1616D GPS 模組（V1 停用，V2 啟用）

```
Pi Pico (腳位)          PA1616D GPS
──────────────          ──────────
3V3OUT (Pin 36) ──────▶ VCC  (3.3V)
GND    (Pin 38) ──────▶ GND
GP8    (Pin 11) ──────▶ RX   ← 接 GPS 的 TX 腳
GP9    (Pin 12) ◀──────  TX   ← 接 GPS 的 RX 腳
                         Baud rate: 9600（GPS_BAUD_RATE 設定）
```
